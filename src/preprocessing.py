...
class Preprocessing:
    ...
    def preprocessApproach25(self, image):
        resized = self.resize_img(image)
        return self.extract_melanoma_blob_no_black(resized)
    ...
    def extract_melanoma_blob_no_black(self, original_image, k=2):
        black_threshold = 5  
        non_black_mask = cv2.inRange(original_image, (0, 0, 0), (black_threshold, black_threshold, black_threshold))
        
        # Invert the mask to identify non-black regions
        non_black_mask = cv2.bitwise_not(non_black_mask)
        
        # Apply K-Means clustering to non-black regions
        pixels = original_image.reshape((-1, 3))
        pixels = np.float32(pixels)
        ...
        # Create a mask for the melanoma cluster
        melanoma_mask = (labels == melanoma_label).reshape(original_image.shape[:2])

        # Apply the non-black mask to restrict the segmentation to non-black regions
        melanoma_mask = cv2.bitwise_and(melanoma_mask.astype(np.uint8), non_black_mask)
        
        # Apply the melanoma mask to the original image
        result_image = cv2.bitwise_and(original_image, original_image, mask=cv2.bitwise_not(melanoma_mask.astype(np.uint8)*255))

        return result_image, melanoma_mask
